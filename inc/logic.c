...
void do_pult_logic_internal(void)
{
  ...
  switch(logic_state) {
    ...
    case 6:
      if (xnacc_poweron) {
        if (io_get_change_on(11)) logic_turnout_operate(ADDR_OSV1);   // operate osvětlení 1
        if (io_get_change_on(12)) logic_turnout_operate(ADDR_OSV2);   // operate osvětlení 2
        if (io_get_change_on(13)) logic_turnout_operate(ADDR_OSV3);   // operate osvětlení 3
      }
      break;
    case 7:
      if (io_get_change_on(14)) {
        xnacc_turnout_action(129, 1);   // operate rozpojovač 1 on
      }
      if (io_get_change_off(14)) {
        xnacc_turnout_action(129, 2);   // operate rozpojovač 1 off
      }
      if (io_get_change_on(15)) {
        xnacc_turnout_action(130, 1);   // operate rozpojovač 2 on
      }
      if (io_get_change_off(15)) {
        xnacc_turnout_action(130, 2);   // operate rozpojovač 2 off
      }
      if (io_get_change_on(16)) {
        xnacc_turnout_action(131, 1);   // operate rozpojovač 3 on
      }
      if (io_get_change_off(16)) {
        xnacc_turnout_action(131, 2);   // operate rozpojovač 3 off
      }
      if (io_get_change_on(17)) {
        xnacc_turnout_action(132, 1);   // operate rozpojovač 4 on
      }
      if (io_get_change_off(17)) {
        xnacc_turnout_action(132, 2);   // operate rozpojovač 4 off
      }
      break;
    case 8:
      if (io_get_change_on(18)) {
        xnacc_turnout_action(133, 1);   // operate rozpojovač 5
      }
      if (io_get_change_off(18)) {
        xnacc_turnout_action(133, 2);   // operate rozpojovač 5
      }
      if (io_get_change_on(19)) {
        xnacc_turnout_action(134, 1);   // operate rozpojovač 6
      }
      if (io_get_change_off(19)) {
        xnacc_turnout_action(134, 2);   // operate rozpojovač 6
      }
      break;
  }
...
void do_pult_logic(void)
{
  ...
  if (logic_flag_10Hz) {
    ...
    if (xnacc_ccavail) {
      ...
      if (logic_inittimer == 5) {
        ...
        xnacc_feedback_request(137); // světla 1
        xnacc_feedback_request(138); // světla 2
        xnacc_feedback_request(139); // světla 3
      }
      // 4 - reserved
      ...
      if (logic_inittimer == 2) {
        ...
        if (sum > 3) logic_inittimer = 0; // pokud máme nějaký stav, tak přestaneme s inicializací (centrála není po restartu)
      }
      if (logic_inittimer == 1) {
        xnacc_turnout_action(4, 1); // nastavit přestavník s adresou 4 do +
  ...
  if ((!xnacc_poweron) && (logic_inittimer > 0)) {
    // není DCC a neproběhla celá inicializace
